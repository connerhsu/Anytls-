...
# ====================================================
# AnyTLS-Go 终极管理版 (默认 8443 + 自定义 SNI + 核心管理)
# ====================================================
...
show_menu() {
    ...
    read -p " 请选择: " num
    case "$num" in
        1) check_sys; install_deps; install_core; configure; show_result; read -p "回车返回..." ;;
...
uninstall() {
    ...
    if [[ "$confirm" == "y" || "$confirm" == "Y" ]]; then
        ...
        rm -rf "$INSTALL_DIR" "$CONFIG_DIR" "$SERVICE_FILE" "/usr/bin/anytls" "/usr/local/bin/anytls"
        print_ok "卸载成功"
    fi
}
...
# 快捷启动
if [[ "$1" == "install" ]]; then
    check_sys; install_deps; install_core; configure; show_result
else
    # 建立快捷指令
    [[ ! -f "$SHORTCUT_BIN" ]] && cp "$0" "$SHORTCUT_BIN" 2>/dev/null && chmod +x "$SHORTCUT_BIN"
    show_menu
fi
