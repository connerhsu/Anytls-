...
AUTO_PAD_SCRIPT="${CONFIG_DIR}/auto_padding.sh" # 自动更换Padding的脚本路径
TZ_DEFAULT="Asia/Shanghai" # 默认时区
SHELL_VERSION="0.1.5" # 脚本版本 (已更新)
...
os_install() {
  if command -v apt-get >/dev/null 2>&1; then
    apt-get update -y
    apt-get install -y ca-certificates unzip curl cron
  elif command -v dnf >/dev/null 2>&1; then
    dnf update -y
    dnf install -y ca-certificates unzip curl cronie
  elif command -v yum >/dev/null 2>&1; then
    yum update -y
    yum install -y ca-certificates unzip curl cronie
  else
    echo "未识别的包管理器，请手动安装 ca-certificates、unzip、curl、cron 后重试"
    exit 1
  fi
  # 确保cron服务启动
...
# 检查定时任务状态
auto_padding_status() {
  if crontab -l 2>/dev/null | grep -q "$AUTO_PAD_SCRIPT"; then
    echo -e "${Green}已开启 (每天 04:30)${Font}"
  else
    echo -e "${Red}已关闭${Font}"
  fi
}
...
toggle_auto_padding() {
  if ! is_installed; then
    print_error "请先安装 AnyTLS"
    return
  fi

  if crontab -l 2>/dev/null | grep -q "$AUTO_PAD_SCRIPT"; then
    # 如果已存在，则关闭
    echo "正在关闭每日自动更换 Padding..."
    crontab -l 2>/dev/null | grep -v "$AUTO_PAD_SCRIPT" | crontab -
    rm -f "$AUTO_PAD_SCRIPT"
    print_ok "已关闭自动更换功能"
  else
    # 如果不存在，则开启
    echo "正在开启每日自动更换 Padding (时间: 04:30)..."
    
    # 1. 创建执行脚本
    cat > "$AUTO_PAD_SCRIPT" <<EOF
#!/bin/bash
# Auto generated by AnyTLS script
CONFIG_FILE="${ANYTLS_CONFIG_FILE}"
LOG_FILE="${CONFIG_DIR}/padding_change.log"

if [ -f "\$CONFIG_FILE" ]; then
    # 生成 100-1500 随机数
    NEW_PADDING=\$((RANDOM % 1400 + 100))
    # 修改配置文件
    sed -i -E "s/padding: [0-9]+/padding: \$NEW_PADDING/" "\$CONFIG_FILE"
    # 重启服务
    systemctl restart ${ANYTLS_SERVICE_NAME}
    echo "\$(date): Padding changed to \$NEW_PADDING" >> "\$LOG_FILE"
fi
EOF
    chmod +x "$AUTO_PAD_SCRIPT"

    # 2. 添加到 Crontab (保留原有任务)
    (crontab -l 2>/dev/null; echo "30 4 * * * /bin/bash $AUTO_PAD_SCRIPT") | crontab -
    
    print_ok "已开启自动更换功能"
    print_info "日志文件位置: ${CONFIG_DIR}/padding_change.log"
  fi
}
...
main() {
  ...
  while true; do
    ...
    echo -e " 自动更换Padding: $(auto_padding_status)"
    hr
    ...
    echo -e "${Cyan}8. 设置/取消 每日自动更换Padding${Font}"
